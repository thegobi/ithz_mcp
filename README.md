...
3. The agent downloads or clones this package repository, expands `ithz_mcp-v0.1-product-candidate-20260530.3.zip` or the latest alias `ithz_mcp-v0.1-product-candidate.zip`, runs smoke checks, initializes the current project, reuses existing `project.ithz` if present, and configures local MCP/Git-driver support.
...
Build `product-candidate-agent-intake.20260530.3` adds first-install agent intake and idempotent reinstall behavior. If Codex CLI is available, it performs a read-only project analysis and stores sanitized workflow/decision/gate/risk memory in `project.ithz`. If Codex CLI is missing, `ithz_mcp.md` instructs the installing host agent to finish intake manually.
